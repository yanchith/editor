...
- Small and simple, so I can make modifications with understanding, and maybe other people can too.
- Fast, unless it conflicts too much with the previous point.
- Preserve the nice parts of my Emacs experience.
- No three-letter acronyms ending with P.
...
2) Modify the code and compile your own build. The catch is that the editor is written in Jai,
   which is not publicly available yet, but hopefully should be out before too long.
   This road is also fraught, because I might have failed at my simplicity goal.
...
I plan to add support for Odin, Rust, C#, Java, Python, Typescript, HLSL and Slang pretty soon.
If you want to add support for a language for yourself, look at the existing `lang_xyz.jai` files.

### Speed

- Current binary releases have expensive paranoid assertions enabled. These help debug issues in tricky
  parts of the code. They are also extremely slow. You can disable these with `-no-sanity`.
  In practice, none of these asserts went off in a long time, so they will default to disabled eventually.

- The parser for indentation and lexers for syntax highlighting are currently much slower than they should be.
  The editor turns them off for files above 10 megabytes.

- Search needs to be faster. Much faster.

Once these are solved, you should be able to view and edit multi-gigabyte files without dropping a frame.

### Misc

- Project and directory search currently shells out to ripgrep, so you need to have the `rg` command available on your system.
  Near-term, I'd like to support more external search tools (e.g. `git grep`).
  Long-term, I want to implement search myself and do away with all of this.

- Spawning a process and collecting its output (for compilation or project search) is currently synchronous.

## Building from source

Run `jai [-x64] build.jai` for a debug build.
Run `jai -optimized_debug build.jai` for a release build.
...
I am not generally prepared to maintain an open-source project. Feel free to make the editor
yours by changing your version.

However, I might accept the occasional patch, especially if it is a small bugfix.
Also probably safe to send lexers for more programming languages.
